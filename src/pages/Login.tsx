...
const ROLE_LABEL_KEY: Record<string, string> = {
  coach: 'nav.role_coach', student: 'nav.role_student', super_admin: 'nav.role_super_admin',
}
...
export default function Login() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    if (!em)             return setError(t('login.err_email_required'))
    if (!validEmail(em)) return setError(t('login.err_valid_email'))
    if (!password)       return setError(t('login.err_password_required'))
    ...
    try {
      ...
      if (profileError || !profile) {
        setLoading(false)
        setError(t('login.err_profile_not_found'))
        return
      }
      ...
      setLoading(false)
      setSuccess({ name: profile.name, roleLabel: t(ROLE_LABEL_KEY[role] ?? 'nav.role_coach'), route, first })
      setView('success')

    } catch {
      setLoading(false)
      setError(t('login.err_connection'))
    }
  }
  ...
  async function onSubmitForgot(e: React.FormEvent) {
    ...
    if (!v || !validEmail(v)) return setForgotError(t('login.err_valid_email'))
    ...
    if (error) {
      setForgotError(t('login.err_send_failed'))
      return
    }
  ...
  const brandLines = t('login.brand_headline').split('\n')
    ...
    <div className="k-split" style={{ display: 'flex', flexDirection: 'row', minHeight: '100vh', background: '#FAEEDA' }}>
      ...
      >
        ...
        <div className="k-brand-mid" style={{ position: 'relative' }}>
          <h1 className="k-login-head" style={{ font: `800 42px/1.08 ${FF}`, color: '#fff', margin: 0, letterSpacing: '-1px' }}>
            {brandLines.map((line, i) => <span key={i}>{line}{i < brandLines.length - 1 && <br />}</span>)}
          </h1>
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '48px 40px', background: '#FAEEDA' }}>
        <div className="k-formwrap" style={{ width: '100%', maxWidth: 392 }}>
          ...
          {view === 'login' && (
            <div>
              <h2 style={{ font: `800 27px ${FF}`, color: '#1B2A4A', margin: '0 0 4px', letterSpacing: '-.5px' }}>{t('login.welcome_back')}</h2>
              <p style={{ font: `400 14px ${FF}`, color: '#7c7869', margin: '0 0 26px' }}>{t('login.enter_to_continue')}</p>
              ...
              <form onSubmit={onSubmit}>
                ...
                <div style={{ marginBottom: 13 }}>
                  ...
                  <div style={{ position: 'relative' }}>
                    ...
                    />
                    <button
                      type="button" onClick={() => setShowPassword(v => !v)} aria-label={t('login.show_password')}
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
                  <label style={{ display: 'flex', alignItems: 'center', gap: 7, cursor: 'pointer', userSelect: 'none' }}>
                    ...
                    <span style={{ font: `400 13px ${FF}`, color: '#6b6657' }}>{t('login.remember_me')}</span>
                  </label>
          ...
          {view === 'forgot' && (
            <div>
              ...
              <h2 style={{ font: `800 26px ${FF}`, color: '#1B2A4A', margin: '0 0 4px', letterSpacing: '-.5px' }}>{t('login.forgot_title')}</h2>
              ...
              <form onSubmit={onSubmitForgot}>
                ...
                <button type="submit" style={btnPrimary}>{t('login.send_link')}</button>
              </form>
            </div>
          )}
          ...
          {view === 'sent' && (
            <div style={{ textAlign: 'center' }}>
              ...
              <h2 style={{ font: `800 24px ${FF}`, color: '#1B2A4A', margin: '0 0 8px', letterSpacing: '-.5px' }}>{t('login.check_email_title')}</h2>
              <p style={{ font: `400 14px/1.55 ${FF}`, color: '#7c7869', margin: '0 0 24px' }}>
                {t('login.check_email_body').replace('<1>', '').replace('</1>', '')} <strong style={{ color: '#1B2A4A' }}>{forgotEmail}</strong>
              </p>
          ...
          {view === 'invite' && (
            <div>
              ...
              <h2 style={{ font: `800 24px ${FF}`, color: '#1B2A4A', margin: '0 0 8px', letterSpacing: '-.5px' }}>{t('login.invite_only_title')}</h2>
              <p style={{ font: `400 14px/1.6 ${FF}`, color: '#7c7869', margin: '0 0 22px' }}>
                {t('login.invite_only_desc')}
              </p>
          ...
          {view === 'success' && success && (
            <div style={{ textAlign: 'center' }}>
              ...
              <h2 style={{ font: `800 25px ${FF}`, color: '#1B2A4A', margin: '0 0 6px', letterSpacing: '-.5px' }}>{t('login.success_hi', { name: success.name })}</h2>
              ...
              <p style={{ font: `400 14px/1.5 ${FF}`, color: '#7c7869', margin: '0 0 6px' }}>{t('login.success_logged_in')}</p>
              <div style={{ font: `600 15px ${FF}`, color: '#E8542A', marginBottom: 8 }}>{t('login.success_your_area')}</div>
              {success.first && (
                <p style={{ font: `400 13px/1.5 ${FF}`, color: '#b06a12', background: '#f7ecd9', borderRadius: 9, padding: '9px 12px', margin: '0 0 4px' }}>
                  {t('login.success_first_access')}
                </p>
              )}
