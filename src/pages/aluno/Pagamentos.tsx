...
const PLANS = [
  {
    ...
    label:       'Trimestral',
    amount:      247,
    sub:         '3 cobranças mensais de R$247',
    badge:       'Mais popular',
    saving:      'Economia de R$456/ano',
  },
  {
    code:        'monthly',
    label:       'Mensal',
...
export default function Pagamentos() {
  ...
  async function handleCheckout() {
    ...
    try {
      ...
      const res  = await fetch(`${FNS}/create-payment-link`, {
        ...
        body:    JSON.stringify({
          amount:      plan.amount,
          description: `Mensalidade Kinea — Plano ${plan.label}`,
          redirectUrl,
        }),
      })
  ...
  if (mode === 'verifying') {
    return (
      <div style={{ background: '#F4EFE3', minHeight: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 16, padding: 24 }}>
        ...
        <div style={{ font: `700 16px ${FF}`, color: '#1B2A4A' }}>Verificando pagamento…</div>
  ...
  if (mode === 'timeout') {
    return (
      <div style={{ background: '#F4EFE3', minHeight: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 14, padding: 24 }}>
        ...
        <div style={{ font: `700 16px ${FF}`, color: '#1B2A4A', textAlign: 'center' }}>Confirmação pendente</div>
        <div style={{ font: `400 13px ${FF}`, color: '#9a948a', textAlign: 'center', maxWidth: 280, lineHeight: 1.6 }}>
          O pagamento está sendo processado. Assim que confirmado, seu acesso será liberado automaticamente.
        </div>
  ...
  if (mode === 'active' && activePayment) {
    ...
    const methodLabel = method === 'pix' ? 'Pix' : method === 'credit_card' ? 'Cartão de crédito' : null
    return (
        ...
        <div style={{ margin: '16px 18px', background: 'linear-gradient(135deg,#1B7a4a,#1a5c39)', borderRadius: 20, padding: 24, position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ position: 'relative' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
              ...
              <span style={{ font: `700 12px ${FF}`, color: 'rgba(255,255,255,.8)', textTransform: 'uppercase', letterSpacing: '.3px' }}>Pagamento confirmado</span>
            </div>
            <div style={{ font: `800 26px ${FF}`, color: '#fff', letterSpacing: '-.5px', marginBottom: 4 }}>
              {fmtMoney(activePayment.amount)}<span style={{ font: `400 14px ${FF}`, opacity: .7 }}>/mês</span>
            </div>
            <div style={{ font: `400 12px ${FF}`, color: 'rgba(255,255,255,.65)' }}>
              {activePayment.description ?? 'Mensalidade Kinea'}
              {activePayment.paid_at && ` · pago em ${fmtDate(activePayment.paid_at)}`}
      ...
      <div style={{ background: '#F4EFE3', minHeight: '100%', paddingBottom: 32 }}>
        ...
        <div style={{ margin: '16px 18px', background: '#fff', borderRadius: 16, padding: 22, boxShadow: '0 2px 8px rgba(27,42,74,.07)', border: '1px solid #ece7d9' }}>
          ...
          <div style={{ font: `800 22px ${FF}`, color: '#E8542A', marginBottom: 16 }}>
            {fmtMoney(pendingPayment.amount)}<span style={{ font: `400 13px ${FF}`, color: '#9a948a' }}>/mês</span>
          </div>
    ...
    <div style={{ background: '#F4EFE3', minHeight: '100%', paddingBottom: 40 }}>
          ...
          return (
            <button key={p.code} type="button" onClick={() => setSelectedPlan(p.code)}
              ...
              <div style={{ textAlign: 'right', flexShrink: 0 }}>
                ...
                <div style={{ font: `400 10.5px ${FF}`, color: '#9a948a' }}>/mês</div>
              </div>
            </button>
          )
        })}
      ...
      <div style={{ position: 'fixed', bottom: 0, left: '50%', transform: 'translateX(-50%)', width: '100%', maxWidth: 390, padding: '12px 18px 24px', background: '#F4EFE3', borderTop: '1px solid #EDE8DC' }}>
        <button type="button" onClick={() => void handleCheckout()} disabled={creating}
          ...
          {creating
            ? <><span style={{ width: 18, height: 18, border: '2.5px solid rgba(255,255,255,.4)', borderTopColor: '#fff', borderRadius: '50%', display: 'inline-block', animation: 'kspin .7s linear infinite' }} />Aguarde...</>
            : `Assinar plano ${plan.label} · ${fmtMoney(plan.amount)}/mês`
          }
        </button>
